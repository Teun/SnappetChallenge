...
import * as action from 'app/store/actions/identity/user.action';
...
export class UserEffects {
  ...
  load$: Observable<Action> = this.actions$
    ...
    .switchMap(_ => {
      ...
      return this.service.Read()
        .takeUntil(next$)
        .map(result => new action.ReadCompleteAction(new Filter <Nicollas.Dto.Identity.userDto, string> (result)))
  ...
  Create$: Observable<Action> = this.actions$
    ...
    .switchMap(entity => {
      ...
      return this.service.Create(entity, entity.roles[0].roleId)
        .takeUntil(next$)
  ...
  ResetPassword$: Observable<Action> = this.actions$
    .ofType(action.RESET_PASSWORD) // the action type that will hit this
    .map((action: action.ResetPasswordAction) => action.payload)
    .switchMap(entity => {
      const next$ = this.actions$.ofType(action.RESET_PASSWORD).skip(1);

      return this.service.ResetPassword(entity)
        .takeUntil(next$)
        .map(result => new action.PasswordCompleteAction())
        .catch((err) => of(new action.FaliedAction(err)));
    });

  @Effect()
  ChangePassword$: Observable<Action> = this.actions$
    .ofType(action.CHANGE_PASSWORD) // the action type that will hit this
    .map((action: action.ChangePasswordAction) => action.payload)
    .switchMap(data => {
      const next$ = this.actions$.ofType(action.RESET_PASSWORD).skip(1);

      return this.service.ChangePassword(data.entity, data.newPassword, data.current)
        .takeUntil(next$)
        .map(result => new action.PasswordCompleteAction())
        .catch((err) => of(new action.FaliedAction(err)));
    });
