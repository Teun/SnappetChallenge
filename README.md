...
```
https://github.com/meetkosiso/SnappetChallenge.git
```

Import the document into snappetTestDB database
```
mongoimport --jsonArray --db snappetTestDB —collection recorded_answers < Data/work.json
```

Convert SubmitDateTime to BsonDateTime by running this script on Mongo shell
```
var cursor = db.recorded_answers.find();
while (cursor.hasNext()) {
var doc = cursor.next();
db.recorded_answers.update({_id : doc._id}, {$set : {SubmitDateTime: new Date(doc.SubmitDateTime)}});
}
```
Start the Visual Studio 2017 and run the solution
