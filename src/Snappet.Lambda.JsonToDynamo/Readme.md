...
WORKDIR /src
COPY ["Snappet.Lambda.JsonToDynamo.csproj", "Snappet.Lambda.JsonToDynamo/"]
RUN dotnet restore "Snappet.Lambda.JsonToDynamo/Snappet.Lambda.JsonToDynamo.csproj"

WORKDIR "/src/Snappet.Lambda.JsonToDynamo"
COPY . .
RUN dotnet build "Snappet.Lambda.JsonToDynamo.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Snappet.Lambda.JsonToDynamo.csproj" \
...
Execute unit tests
```
cd "Snappet.Lambda.JsonToDynamo/test/Snappet.Lambda.JsonToDynamo.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "Snappet.Lambda.JsonToDynamo/src/Snappet.Lambda.JsonToDynamo"
