...
namespace SnappetChallenge.Core.Services
{
    public class AssessmentService : IAssessmentService
    {
        ...
        public IEnumerable<Assessment> GetFilterdAssessments(AssessmentFilterModel assessmentFilterModel)
        {
        ...
        private void AppendFilters(AssessmentFilterModel assessmentFilterModel, List<Expression<Func<Assessment, bool>>> filters)
        {
            ...
            {
                if (bool.Parse(assessmentFilterModel.Correct))
                {
                    filters.Add(c => c.Correct);
                }
                else
                {
                    filters.Add(c => !c.Correct);
            ...
            {
                int filteValue = int.Parse(assessmentFilterModel.Progress);
                filters.Add(c => c.Progress == filteValue);
            }
        ...
        public IEnumerable<ClassResult> GetClassResults(string submitDateTime = "2015-03-24 11:30:00")
        {
            List<Expression<Func<Assessment, bool>>> filters = new List<Expression<Func<Assessment, bool>>>();
            var assessmentFilterModel = new AssessmentFilterModel();

            AppendFilters(new AssessmentFilterModel { SubmitDateTime = submitDateTime }, filters);

            var result = _unitOfWork.AssessmentQueries.Query(filters);
            var groupedResult = result.GroupBy(x => x.UserId)
                .Select(c =>
                    new ClassResult
                    {
                        UserId = c.Key,
                        NumberOfQuestions = c.Count(x => x.SubmittedAnswerId > 0),
                        IncorrectAnswers = c.Count(x => !x.Correct),
                        CorrectAnswers = c.Count(x => x.Correct),
                        NumberOfExercises = c.Select(x => x.ExerciseId).Distinct().Count(),
                        TotalProgress = c.Sum(x => x.Progress)
                    });
