...
namespace SnappetChallenge.Application.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult ClassResults(string submitDateTime)
        {
         
                DateTime temp;
                IEnumerable<ClassResult> result;
                if (string.IsNullOrEmpty(submitDateTime) || !DateTime.TryParse(submitDateTime,out temp))
                {
                     result = _assessmentService.GetClassResults();
                }
                else 
                {
                    result = _assessmentService.GetClassResults(submitDateTime);
                }

                return new JsonResult
                {
                    Data = new DataResponse<ClassResult>
                    {
                        List = result,
        ...
        {
            var result = _assessmentService.GetFilterdAssessments(assessmentFilterModel).Select(c => new AssessmentFilterModel()
            {
                Correct = c.Correct.ToString(),
