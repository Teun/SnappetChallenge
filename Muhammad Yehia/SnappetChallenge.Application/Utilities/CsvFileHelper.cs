...
namespace SnappetChallenge.Application.Utilities
{
    public static class CsvFileHelper
    {
        ...
        private static void InitializeInsertStatment(out int index, out StringBuilder insertStatment, out bool firstAppend)
        {
            ...
            insertStatment =
                new StringBuilder(
                    "INSERT INTO [dbo].[Assessment] ( [SubmittedAnswerId], [SubmitDateTime], [Correct], [Progress], [UserId], [ExerciseId], [Difficulty], [Subject], [Domain],[LearningObjective])");
            firstAppend = true;
        }
        ...
        {
            if (readLineArray.Length != 10) return;
            var insertTemplate =
                $" select N'{readLineArray[0]}',N'{readLineArray[1]}',N'{readLineArray[2]}',N'{readLineArray[3]}',N'{readLineArray[4]}',N'{readLineArray[5]}',N'{readLineArray[6]}',N'{readLineArray[7]}',N'{readLineArray[8]}',N'{readLineArray[9]}'";
        ...
        public static void ReplaceCommaInsideDoubleQoutString(ref string readLine)
        {
            ...
            for (int i = 0, j = 0; i < readLine.Length; i++)
            {
                ...
                else if (readLine[i] == '\"' &&(i + 1 == readLine.Length ||readLine[i + 1] == ',') && i > j)
                {
